...
class VisitSensor:
        """
        Simulate a sensor at the entrance of a mall.
        ...
        def simulate_visit_count(self, business_date: date) -> int:
            ...
            np.random.seed(seed=business_date.toordinal()) #avoir toujours le même chiffre et donc toujours la même seed pour un jour donné


            # Find out which day the business_date corresponds to: Monday = 0, Sunday = 6
            week_day = business_date.weekday()

            visit = np.random.normal(self.avg_visit, self.std_visit) # on simule une distribution normale
        ...
        def get_visit_count(self, business_date: date) -> int:
            ...
            if proba_malfunction < self.perc_malfunction:
                print("malfunction")
                visit = np.floor(visit * 0.2) # lake it so bad we can detect it
